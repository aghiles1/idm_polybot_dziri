...
public class PolyBrain extends PolyRob{
	...
	public boolean bombDetected() {
		if(this.hasDetectedAnObject()) {
			int objDist = this.getDetectedObjectDistance();
			Position2D objectCoordinate = new Position2D(
												(int)Math.round(this.getPosition().x+Math.cos(this.getOrientation())*objDist),
												(int)Math.round(this.getPosition().y+Math.sin(this.getOrientation())*objDist)
										  );
			System.out.println("add obstacle "+objectCoordinate.x+","+objectCoordinate.y);
			boolean addIt = true;
			addIt = checkIfDetectedObjectIsAPaintBomb(this.getViewableBlobs(), objectCoordinate, addIt);
			if (addIt) {
				pathToCompute = true;
				boundCoordinate(objectCoordinate);
				System.out.println("add obstacle "+objectCoordinate.x+","+objectCoordinate.y);
	...
	protected boolean checkIfDetectedObjectIsAPaintBomb(ArrayList<Blob> blobs, Position2D objectCoordinate,boolean addIt) {
		for(Blob b: blobs) {
			if ((Math.abs(b.positionX - objectCoordinate.x) <= 500) && (Math.abs(b.positionY - objectCoordinate.y) <= 500)) {
	...
	public void smoothMoveToTarget() {
		if (!this.hasDetectedAnObject()) {
			this.turnRight(3);
			return;
		}
	...
	public void goToSafePlace(Position2D robPos, float robOrientation) {
		if ((Math.abs(safePlace.x - robPos.x) <= 100) && (Math.abs(safePlace.y - robPos.y) <= 100)) {
